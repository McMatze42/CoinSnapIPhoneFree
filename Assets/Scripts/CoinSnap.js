...
private var barrelPos2: Vector3 = Vector3(1.64, 0, 2.0);
...
private var namePlayer2: String = "Comp #1";
private var namePlayer3: String = "Comp #2";
private var namePlayer4: String = "Comp #3";
...
function setStateNewGame()
{
	...
	{ 
		...
		countPlayer = countCompPlayer + 1; // 1 human
...
function StateNewGame(state:int)
{
	if (!arState[state])
	{
		...
		//if (barrelClone2 != null) Destroy(barrelClone2);
		...
		switch (gameLevel)
		{
			...
			case 6:
				wallObject.renderer.material = wallMats[1];
				if (barrelClone1 != null) Destroy(barrelClone1);
				placeBarrel2();
			break;
			case 7:
				wallObject.renderer.material = wallMats[2];
				if (barrelClone2 != null) Destroy(barrelClone1);
				placeBarrel1();
			break;
			case 8:
				wallObject.renderer.material = wallMats[3];
...
function StateNextPlayer(state:int)
{
	if (!arState[state])
	{
		...
		if (actualPlayer < countPlayer)
		{
			...
			if (actualPlayer < 1) // must be human
			{
